class EmotionFeaturizer():
    """Get emotion features for English
    
    Combine featurization of language independent emotion feature package with English sentiment feature package
    ...
    """
    def __init__(self, nlp, name, path='', features=['valence','arousal','dominance','joy','anger','sadness','fear','disgust','polarity','subjectivity'], level='document',
        ...
        self._modules = []
        emo_features = [feature for feature in features if feature not in ['polarity','subjectivity']]
        if emo_features:
            from ....emotion import EmotionFeaturizer
            self._modules.append(EmotionFeaturizer(nlp, 'en', name, path=path, features=emo_features, level=level))
        if any (feature in features for feature in ['polarity','subjectivity']):
            from .sentiment import SentimentFeaturizer
            self._modules.append(SentimentFeaturizer(features=[feature for feature in ['polarity','subjectivity'] if feature in features], level=level))

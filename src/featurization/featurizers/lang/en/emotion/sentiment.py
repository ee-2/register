...
class SentimentFeaturizer():
    """Get sentiment for English

    Used in combination with emotion featurizer, not as standalone feature package (no own config)

    Parameters
    ----------
    features : list, default=['polarity','subjectivity']
        'polarity': TextBlob's polarity score
        'subjectivity': TextBlob's subjectivity score
    level : {'token','sentence','document'}
        level to calculate formality on, default is 'token'

    Notes
    -----
    Uses TextBlob's polarity and subjectivity score (https://textblob.readthedocs.io/en/latest/)
    """
    def __init__(self, features=['polarity', 'subjectivity'], level='document'):

...
class Classification_tf_keras(Classification):
    """Simple keras (tensorflow) model for classification
    ...
    """
    def __init__(self, dim, scalers, nr_labels, model='NeuralNetwork', hidden_layer_dimensions=[128,64], nr_hidden_layers=0, dropout_input=0.2, dropout_hidden=0.5, path=None, **config):
        self._scalers = scalers
        self._nr_labels= nr_labels
        if path:
            ...
            # output layer
            self.model.add(keras.layers.Dense(self._nr_labels, activation='softmax', kernel_initializer='random_normal', bias_initializer='random_normal'))
    ...
    def fit(self, x_train, y_train):
        ...
        y_train, y_dev = keras.utils.to_categorical(y_train[0], num_classes=self._nr_labels), keras.utils.to_categorical(y_train[1], num_classes=self._nr_labels)
    ...
    def predict(self, x_test):
        ...
        array_like
        """
        return np.argmax(self.model.predict_step(x_test), axis=-1)
